...
```bash
python3 -B -m unittest -v test_jev.py test_web.py test_history.py
```

## 无痕与记忆模式

每次打开或刷新网页默认进入**无痕模式**，不会自动读取历史。切换模式会清空当前输入、草稿和结果，已保存的历史不受影响。

- **无痕模式**：本工具不将本次文本、问题或结果写入历史文件，也不写入浏览器 localStorage / Cookie。关闭或刷新页面后，本次内容不保留。
- **记忆模式**：仅保存成功判断的文本、问题、类型、选项或等级、结果和时间。刷新网页或重启服务后，重新选择“记忆”即可查看。草稿和失败请求不保存。
- **每次判断仍独立**：历史不会自动发送给 Jev。点击“载入并查看”只恢复输入和当时的结果，不调用 API；修改后点击“开始判断”才会重新调用并生成一条新记录。
- 历史列表每页显示 20 条，支持单条删除和清空全部历史；删除历史不会删除 API Key。

历史以明文保存在运行 Python 的用户目录 `~/.local/share/jev-local/history.sqlite3`，位于项目之外；WSL 下目录权限 700、文件权限 600，Windows 使用用户目录的系统权限。数据不上传 GitHub，API Key 不作为历史字段保存。WSL 与 Windows 的历史目录独立；本机多个浏览器连接同一服务时共用这份历史。

这里的“无痕”指本工具不保存本次历史；执行判断时，文本和问题仍会发送至 TypeSafe API。API Key 的保存设置独立于历史模式。若判断成功但历史写入失败，页面会保留结果并提示未保存。
...
- 仅网页记忆模式保存成功判断的输入和结果；无痕模式和命令行版不保存。工具不修改 Agent 配置。`.gitignore` 排除常见密钥文件及历史数据库。
